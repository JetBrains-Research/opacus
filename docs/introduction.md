...
**Non-wrapping mode:** Set `wrap_model=False` to attach hooks directly to your model without wrapping. This preserves model type, keeps clean state dicts, and provides better compatibility with transformer models. Requires manual cleanup via `model._opacus_hooks.cleanup()` when done.
