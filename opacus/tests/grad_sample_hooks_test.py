...
class GradSampleHooksTest(unittest.TestCase):
    ...
    def test_zero_grad(self):
        ...
        x, _ = next(iter(self.dl))
        self.hooks_model = self.hooks_model.train()
        hooks_out = self.hooks_model(x)
        loss = self.criterion(hooks_out, torch.zeros_like(hooks_out))
        loss.backward()

        # Clear grad_sample manually (hooks don't have zero_grad method)
        for p in self.hooks_model.parameters():
            if hasattr(p, "grad_sample"):
                p.grad_sample = None

        params_with_gs = [
            n
            for n, p in self.hooks_model.named_parameters()
...
class FastGradientHooksTestUnit(GradSampleHooksTest):
    """Test FastGradientHooks - hooks with ghost clipping."""

    CLS = FastGradientHooks
    ...
    def test_get_norm_sample(self):
        ...
        # Get norms
        norms = self.grad_sample_hooks.get_norm_sample()
    ...
    def test_get_clipping_coef(self):
        ...
        coeff = self.grad_sample_hooks.get_clipping_coef()
